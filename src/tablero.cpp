...
tablero::tablero( int dim ){
	...
	for(int i = 0; i < dim; i++ ) {
		tabla[i]= new casilla[dim]; //(420 / dim);
		for(int j=0; j < dimension; j++) {
			tabla[i][j].setDimension(420 / dim);
		}
	}
}
...
void tablero::imprimir_tablero( SDL_Renderer* dibujante ){

	for (int j=0; j<dimension; j++) {
		for (int i= 0; i<dimension; i++) {
			//std::cout<< "\t pos[" << i << "][" << j << "]->" << tabla[i][j].ver_valor() ;// << "\t";
			tabla[i][j].dibujarCasilla(i, j, dibujante);
